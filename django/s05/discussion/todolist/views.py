...
def index(request):
     ...
     eventlist_list = EventList.objects.filter(user_id = request.user.id)
...
def eventlist(request, event_id):
    event = get_object_or_404(EventList, pk=event_id)
    context = model_to_dict(event)
    return render(request, "todolist/eventlist.html", context)
...
def add_Event(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() == False:
            ...
            # Checks the database if a task already exists
            # By default the "filter" method searches for records that are case insensitive
            duplicates = EventList.objects.filter(event_name=event_name, user_id = request.user.id)
            
            # if todoitem does not contain any duplicates
            if not duplicates:

                # Creates an object based on the "ToDoItem" model and saves the record in the database
                EventList.objects.create(event_name=event_name, description=description, event_date=timezone.now(), user_id = request.user)
                return redirect("todolist:index")
